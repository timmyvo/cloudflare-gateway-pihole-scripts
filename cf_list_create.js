...
fs.readFile('input.csv', 'utf8', async (err, data) => {
  ...
  if (domains.length > LIST_ITEM_LIMIT) {
    ...
    await console.warn(`More than ${LIST_ITEM_LIMIT} domains found in input.csv - input has to be trimmed`);
  }
...
async function createZeroTrustList(name, items, currentItem, totalItems) {
  ...
  console.log(`Creating "${name}" with ID ${listId} - ${totalItems - currentItem} left`);
}
